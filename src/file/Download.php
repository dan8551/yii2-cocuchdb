 ...
 *
 * ```php
 * Yii::$app->couchdb->getFileCollection()->createDownload($document['_id'])->toFile('/path/to/file.dat');
 * ```
 *
 * You can use `Download::substr()` to read a specific part of the file:
 *
 * ```php
 * $filePart = Yii::$app->couchdb->getFileCollection()->createDownload($document['_id'])->substr(256, 1024);
 * ```
 *
 * @property string $bytes File content. This property is read-only.
 * @property ??? $chunkCursor Chuck list cursor. This property is read-only.
...
class Download extends BaseObject
{
    ...
    private $_document;
    /**
     * @var ??? cursor for the file chunks.
     */
